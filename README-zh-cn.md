...
Multer 是一个 node.js 中间件，用于处理 `multipart/form-data` 类型表单数据, which is primarily used for uploading files. It is written
...
Multer 添加一个 `body` 对象 以及 `file` 或 `files` 对象 到 `request` 对象中.  `body` 对象包含表单的文本域信息, `file` 或 `files` 对象包含对象表单上传的文件信息.
...
If you want to catch errors specifically from multer, 你可以自己调用中间件程序.
...
See [the documentation here](/StorageEngine.md) 如果你想要构建自己的存储引擎.
